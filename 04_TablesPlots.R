...
#             caption="Yield curve response to short rate movements on policy days and non-policy days as well as QE announcements.",
...
# regression and fill into table
for(k in 4:(ncol(DeltaYieldCurves)-4)){
  # regress change in n-maturity on NP-dummy*delta3m + End-dummy*delta3m + Ex-dummy*delta3m
  # + QE dummy
  myreg <- lm(DeltaYieldCurves[,k]~DeltaYieldCurves$d3M:DeltaYieldCurves$NP + 
                DeltaYieldCurves$d3M:DeltaYieldCurves$End+
  ...
  Tab_EndvsExdays_QE[2,j]  <- paste0("(", format(unlist(
    formatC(abs(round(summary(myreg)$coef[1,4],2)),format="f",digits=2)
  )),")") # std intercept
  Tab_EndvsExdays_QE[3,j]  <- round(summary(myreg)$coef[3,1],2) # beta NP
  Tab_EndvsExdays_QE[4,j]  <- paste0("(", format(unlist(
    formatC(abs(round(summary(myreg)$coef[3,4],2)),format="f",digits=2)
  )),")") # std beta NP
  Tab_EndvsExdays_QE[5,j]  <- round(summary(myreg)$coef[4,1],2) # beta End
  Tab_EndvsExdays_QE[6,j]  <- paste0("(", format(unlist(
    formatC(abs(round(summary(myreg)$coef[4,4],2)),format="f",digits=2)
  )),")") # std beta End
  Tab_EndvsExdays_QE[7,j]  <- round(summary(myreg)$coef[5,1],2) # beta Ex
  Tab_EndvsExdays_QE[8,j]  <- paste0("(", format(unlist(
    formatC(abs(round(summary(myreg)$coef[5,4],2)),format="f",digits=2)
  )),")") # std beta Ex
  Tab_EndvsExdays_QE[9,j]  <- round(summary(myreg)$coef[2,1],2) # beta QE
  Tab_EndvsExdays_QE[10,j]  <- paste0("(", format(unlist(
    formatC(abs(round(summary(myreg)$coef[2,4],2)),format="f",digits=2)
  )),")") # std beta QE
...
#             caption="Yield curve response to short rate movements on classified policy days after controlling for QE announcements.",
