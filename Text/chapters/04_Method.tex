...
% ME
%
The ME classifier, on the other hand, directly predicts the sentiment along with its corresponding probability based on the trained model. Comparing the absolute number of classified articles and simply selecting the one with the higher amount yields six endogenous, 27 exogenous and one ambiguous target rate adjustment. 
%
% KNN
%
Similarly, knn directly outputs the classification of each article based on the most likely affiliation due to terms in its immediate proximity. Taking a simple majority vote, knn classifies two dates as endogenous and 32 as exogenous; compared to ME, there are less close calls. 
%
% SVM
%
SVM, finally, yields direct classifications with corresponding probabilities based on a logistic distribution using maximum likelihood. In contrast to ME which estimates the empirical probability distribution, SVM probabilities range from $.50$ to $.95$ with a $.75$-quantile of $.55$ while ME probabilities lie between $.51$ and $1.00$ with median close to one. Even though SVM classifies up to 18 articles as exogenous, apart from one occasion which could not unambiguously be classified all dates were determined to be endogenous.
